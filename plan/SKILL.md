...
3. **Execute in one Docker** (required): Run scan steps **only** via **scan-in-docker**: start one container → copy code in → run each selected step’s `scripts` inside the container → copy reports out. Do **not** run `scripts` on the host (e.g. in terminal on host); that does not use Docker. On step failure: record and continue; summarize at end.
